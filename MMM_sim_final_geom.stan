...
data {
  ...
  row_vector[num_params] mta_mn_priors;
  row_vector[num_params] mta_sd_priors;
  row_vector[num_media_inter-1] mta_betainter_mn_priors;
  row_vector[num_media_inter-1] mta_betainter_sd_priors;
  row_vector[num_media_inter-1] mta_kinter_mn_priors;
  row_vector[num_media_inter-1] mta_kinter_sd_priors;

  
  //real<lower=.05, upper=1.> beta_defl_factor;
  //real<lower=.05, upper=1.> alpha_defl_factor;
 // real<lower=1, upper=3.> sd_infl_factor;
...
model {
   //alpha[1] ~ normal(mta_mn_priors[1]*alpha_defl_factor, mta_sd_priors[1]);
  ...
  //delay ~ uniform(0, max_lag - 1);
   ...
   beta_medias[1] ~ normal(mta_mn_priors[2], mta_sd_priors[2]);
  // beta_medias[1] ~ normal(1.03*.1, .25*2);
   beta_medias[2] ~ normal(1, 1);
   beta_medias[3] ~ normal(1, 1);
   
   for (media_index in 1 : num_media_inter-1) {
    //beta_medias_inter[media_index] ~ uniform(-1, 1);
    beta_medias_inter[media_index] ~ normal(mta_betainter_mn_priors[media_index], mta_betainter_sd_priors[media_index]);
   }


   //k[1] ~ normal(mta_mn_priors[3], mta_sd_priors[3]);
   //k[1] ~ normal(.7, .25);
   ...
   for (media_index in 1 : num_media_inter-1) {
    //beta_medias_inter[media_index] ~ uniform(-1, 1);
    k[media_index+num_media] ~ normal(mta_kinter_mn_priors[media_index], mta_kinter_sd_priors[media_index]*1.5);
   }

   //slope[1] ~ normal(mta_mn_priors[4], mta_sd_priors[4]);
